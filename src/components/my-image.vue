    ...
    <el-dialog :visible.sync="dialogVisible" width="750px">
      <el-tabs v-model="activeName" type="card">
        <el-tab-pane label="素材库" name="true">
          ...
          <!-- 图片列表 -->
          <div class="img_list">
            <div class="item" :class="{selected: selectedImageUrl === item.url}" v-for="item in imagesList" :key="item.id" @click="selected(item.url)">
              <img :src="item.url" />
            </div>
          </div>
          <el-pagination
...
.item {
  ...
  &.selected::after {
      ...
      width: 100%;
      height: 100%;
      background: rgba(0,0,0,.3) url(../assets/images/selected.png) no-repeat center / 50px;
  }
}
</style>
